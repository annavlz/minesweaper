...
describe('mockDOMResponse', function() {
  ...
  it('should make an Observable for clicks on `.foo`', function(done) {
    const userEvents = mockDOMResponse({'.foo': {'click': Rx.Observable.just(135)}});
  ...
  it('should make multiple user event Observables', function(done) {
    const userEvents = mockDOMResponse({
      '.foo': {'click': Rx.Observable.just(135)},
      '.bar': {'scroll': Rx.Observable.just(2)}
    });
    Rx.Observable.combineLatest(userEvents.select('.foo').events('click'), userEvents.select('.bar').events('scroll'), (a, b) => a * b).subscribe((ev) => {
      assert.strictEqual(ev, 270);
      done();
    });
  });
  it('should make multiple user event Observables on the same selector', function(done) {
    const userEvents = mockDOMResponse({'.foo': {
        'click': Rx.Observable.just(135),
        'scroll': Rx.Observable.just(3)
      }});
    Rx.Observable.combineLatest(userEvents.select('.foo').events('click'), userEvents.select('.foo').events('scroll'), (a, b) => a * b).subscribe((ev) => {
