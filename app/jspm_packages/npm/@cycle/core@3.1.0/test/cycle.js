...
describe('Cycle', function() {
  ...
  describe('run()', function() {
    ...
    it('should return requests object and responses object', function() {
      function app(ext) {
        return {other: ext.other.take(1).startWith('a')};
      }
      function driver() {
    ...
    it('should return a disposable drivers output', function(done) {
      function app(res) {
        return {other: res.other.take(6).map((x) => String(x)).startWith('a')};
      }
      function driver(req) {
    ...
    it('should happen on event loop\'s next tick', function(done) {
      function app() {
        return {other: Cycle.Rx.Observable.from([10, 20, 30])};
      }
      let mutable = 'wrong';
      function driver(req) {
    ...
    it('should not work after has been disposed', function(done) {
      let number$ = Cycle.Rx.Observable.range(1, 3).concatMap((x) => Cycle.Rx.Observable.just(x).delay(50));
      function app() {
        return {other: number$};
      }
      let [requests, responses] = Cycle.run(app, {other: (number$) => number$.map((number) => 'x' + number)});
    ...
    it('should report errors from main() in the console', function(done) {
      ...
      function main(responses) {
        return {other: responses.other.take(1).startWith('a').map(() => {
