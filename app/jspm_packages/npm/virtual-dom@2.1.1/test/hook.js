...
test("Node child stateless hooks are not identified", function(assert) {
  ...
  var parentNode = new Node("div", {"id": "not a hook"}, [node], undefined, undefined);
...
test("hooks get called in render", function(assert) {
  var counter = 0;
  var vtree = h("div", {"some-key": hook(function(elem, prop) {
      counter++;
...
test("functions are not hooks in render", function(assert) {
  ...
  var vtree = h("div", {"someProp": fakeHook});
  var elem = create(vtree);
...
test("hooks get called in patch", function(assert) {
  var counter = 0;
  var prev = h("div");
  var curr = h("div", {"some-key": hook(function(elem, prop) {
      counter++;
      assert.equal(prop, "some-key");
      assert.equal(elem.tagName, "DIV");
      elem.className = "bar";
...
test("hooks are not called on trivial diff", function(assert) {
  ...
  var vnode = h("div", {test: hook(function() {
      counters.a++;
    })}, [h("div", {test: hook(function() {
      counters.b++;
    })}), h("div", {test: hook(function() {
      counters.c++;
    })})]);
...
test("all hooks are unhooked", function(assert) {
  ...
  Thunky.prototype.render = function() {
    return h("div", {rootHook: thunkyRootHook}, [h("div", {childHook: thunkyChildHookA}), h("div", {childHook: thunkyChildHookB}), h("div", {childHook: thunkyChildHookC})]);
  };
  Thunky.prototype.type = "Thunk";
  var firstTree = h("div", {rootHook: rootHook}, [h("div", {childHook: childHookA}), h("div", {childHook: childHookB}, [new Thunky()]), h("div", {childHook: childHookC})]);
